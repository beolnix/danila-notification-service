...
use crate::api::rest::dto::{CreateNotificationReqeust, StatusResponse};
...
impl RestController {
    pub fn new(storage: Arc<RwLock<storage::Storage>>) -> RestController {
        RestController { storage: storage }
    }

    pub fn get_notifications_for(
        &self,
        device: &String,
    ) -> Box<Future<Item = Response<Body>, Error = hyper::Error> + Send> {
        println!(
            "DEBUG: received GET notifications request for device: {}",
            device
        );
    ...
    pub fn create_notification(
        ...
        if !valid_city {
            ...
            return bad_request_rsp(format!(
                "The city {} is not supported. Supported cities are: {}.",
                &for_city, &supported_cities
            ));
        }
        ...
        match event_type.as_ref() {
            ...
            "MESSAGE" => match req.message_text {
                Some(text) => self.create_text_msg(for_city, text),
                None => bad_request_rsp(String::from(
                    "message_text property must not be missed if notification type is MESSAGE.",
                )),
            },
            _ => bad_request_rsp(format!(
                "The event type '{}' is not supported. Supported types are: SLAP, MESSAGE.",
                &event_type
            )),
        }
    }
...
fn prepare_response(num: usize) -> Box<Future<Item = Response<Body>, Error = hyper::Error> + Send> {
    ...
    match serde_json::to_string(&response_object) {
        ...
        Err(err) => {
            println!(
                "ERROR: failed to serialize response for notification creation: {:?}",
                err
            );
            internal_error_rsp()
        }
